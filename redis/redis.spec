...
Patch0:           %{name}-2.6.10-redis.conf.patch
...
%package sentinel
Summary:        A distributed monitoring system for Redis
Group:          Applications/Databases
License:        BSD
Requires:       redis = %{version}-%{release}

%description sentinel
Redis Sentinel is a distributed monitoring system for Redis. On top of the
monitoring layer it also implements a notification system with a simple to
use API, and an automatic failover solution. 
...
install -p -D -m 644 %{name}.conf %{buildroot}%{_sysconfdir}/sentinel.conf
...
install -m 755 src/%{name}-sentinel %{buildroot}%{_sbindir}/%{name}-sentinel
...
%changelog
* Thu Feb 14 2013 Matthew Jones <matthew@meez.com> - 2.6.10-1
- Update to redis 2.6.10

* Wed Feb 06 2013 Matthew Jones <matthew@meez.com> - 2.6.9-1
